...
final class StackScreenCoordinator: StackScreenCoordinatorDelegate {
    ...
    // MARK: - State
    private(set) weak var stackViewController: StackScreenViewController?
    ...
    func presentOnSlide(
        ...
        let container = StackScreenViewController(rootViewController: rootViewController, coordinator: self)
    ...
    func presentOnSlide(
        ...
        presentOnSlide(screens: [screen], panGesture: panGesture, didDismiss: didDismiss)
    }
    ...
    private func attachGestureManager(to viewController: UIViewController) {
        if let stackVC = self.stackViewController {
            let gestureManager = StackScreenGestureManager(coordinator: self)
...
extension StackScreenCoordinator {
...
extension StackScreenCoordinator {
    private func debugHeroPopState(animatedView: UIView, destinationView: UIView, fromVC: UIViewController, toVC: UIViewController, window: UIWindow) {
        print("\n=========== 🧩 HERO DEBUG STATE (POP) ===========")
        print("🧾 fromVC: \(String(describing: fromVC))")
        print("🧾 toVC: \(String(describing: toVC))")
        
        print("\n📦 Animated View:")
        print("• class: \(type(of: animatedView))")
        print("• frame: \(animatedView.frame)")
        print("• in window: \(animatedView.window != nil)")
        print("• frame in window: \(animatedView.convert(animatedView.bounds, to: window))")
        
        print("\n🎯 Destination View:")
        print("• class: \(type(of: destinationView))")
        print("• frame: \(destinationView.frame)")
        print("• in window: \(destinationView.window != nil)")
        print("• frame in window: \(destinationView.convert(destinationView.bounds, to: window))")
        
        if let cell = destinationView as? UICollectionViewCell {
            print("• Destination is a UICollectionViewCell")
            print("• contentView frame: \(cell.contentView.frame)")
        } else {
            print("• Destination is not a cell")
        }
        
        print("\n📐 VC Frames:")
        print("• fromVC.view.frame: \(fromVC.view.frame)")
        print("• toVC.view.frame: \(toVC.view.frame)")
        print("• toVC.view.transform: \(toVC.view.transform)")
