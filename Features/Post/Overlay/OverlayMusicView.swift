...
final class OverlayMusicView: UIStackView {
    ...
    private func setup() {
        ...
        // Music label
        _musicLabel.font = .systemFont(ofSize: UIFont.preferredFont(forTextStyle: .caption1).pointSize, weight: .medium)
        ...
        config.image = UIImage(systemName: "plus", withConfiguration: UIImage.SymbolConfiguration(pointSize: UIFont.preferredFont(forTextStyle: .caption2).pointSize, weight: .medium))
        config.imagePadding = 2
        config.imagePlacement = .leading
        config.attributedTitle = AttributedString("Add", attributes: AttributeContainer([
            .font: UIFont.systemFont(ofSize: UIFont.preferredFont(forTextStyle: .caption2).pointSize, weight: .medium),
