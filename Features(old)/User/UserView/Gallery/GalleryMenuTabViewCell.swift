...
//            ])
    ...
    final class MenuPillsView: UIView {
        private let blurView = UIVisualEffectView(effect: UIBlurEffect(style: .systemMaterial))
        ...
        private func makeButton(title: String, isSelected: Bool) -> CustomButton {
            let btn = CustomButton()
            btn.titleText = title
            btn.directionalContentInsets = NSDirectionalEdgeInsets(top: 6, leading: 16, bottom: 6, trailing: 16)
        ...
        private func select(_ index: Int) {
            ...
            for (i, pill) in buttons.enumerated() {
                pill.isSelected = (i == index)
            }
            selectedIndex = index
