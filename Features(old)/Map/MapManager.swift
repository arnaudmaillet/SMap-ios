...
final class MapManager: NSObject {
    ...
    // Appelé depuis HomeVC ou autre contrôleur pour attendre l’apparition
    func waitForAnnotationRender(_ annotation: MKAnnotation, completion: @escaping () -> Void) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) { [weak self] in
            if self?.annotationRenderCallbacks[key] != nil {
                print("[MapManager](waitForAnnotationRender) ⏰ Timeout reached, force map interaction enabled et completeTransition")
                self?.annotationRenderCallbacks[key]?()
                self?.annotationRenderCallbacks[key] = nil
            }
        }
    }
