...
// MARK: - DataSource/Delegate pour la collection verticale (barres d’actions)
extension HomeVC: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    func numberOfSections(in collectionView: UICollectionView) -> Int { 1 }
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int { buttonSections.count }
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: HomeActionButtonRowCell.reuseIdentifier, for: indexPath) as! HomeActionButtonRowCell
        let section = buttonSections[indexPath.item]
        let usesSmallCell = indexPath.item < 2
        cell.configure(with: section.actions, activeIndex: section.selectedIndex, usesSmallCell: usesSmallCell) { [weak self] selectedIndex in
            guard let self else { return }
            self.buttonSections[indexPath.item].selectedIndex = selectedIndex
        }
        return cell
    }
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        let isSmallSection = indexPath.item < 2
        return CGSize(width: collectionView.bounds.width, height: isSmallSection ? 36 : 48)
    }
}
